			...
			if (form instanceof EmployeeCommand) {
				...
				try{
					...
					resultSuccess.setMessage(resuleSuccessMessage);
						...
						Boolean isSimilarPassword = employeeDao.checkPassword(username, password, organization);
						employeeDao.resetPassword(username,password,organization);
						resultSuccess.setMessage("Password reset successfully");
						if(isSimilarPassword == Boolean.TRUE){
							resultSuccess.setData(isSimilarPassword);
						}else{
							employeeDao.resetPassword(username, password, organization);
						}
					}
