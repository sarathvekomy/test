...
public class EmployeeDao extends BaseDao {
	...
	public List<EmployeeResult> searchEmployee(EmployeeCommand command,String logggedinUser,
		...
		if (command != null) {
			String name = command.getFirstName();
			String userName = command.getUsername();
			String employeeType = command.getEmployeeType();
			if (!StringUtils.isEmpty(name)) {
				String nameArray[] = name.split(" ");
				if(nameArray.length == 1){
					Criterion firstName = Expression.like("firstName", name, MatchMode.START).ignoreCase();
					Criterion lastName = Expression.like("lastName", name, MatchMode.START).ignoreCase();
					Criterion middleName = Expression.like("middleName", name, MatchMode.START).ignoreCase();
					Junction conditionGroup = Restrictions.disjunction();
					conditionGroup.add(firstName).add(lastName).add(middleName);
					criteria.add(conditionGroup);
				}else if(nameArray.length == 2){
					criteria.add(Restrictions.eq("firstName",nameArray[0]))
					.add(Restrictions.eq("lastName",nameArray[1]));
				}else if(nameArray.length == 3){
					criteria.add(Restrictions.eq("firstName",nameArray[0]))
					.add(Restrictions.eq("middleName",nameArray[1]))
					.add(Restrictions.eq("lastName",nameArray[2]));
				}
			}
	...
	/**
	 * This mwthod is reponsible to reset the particular employee's password
	 * 
	 * @param username
	 * @param password
	 * @param organization
	...
	public void resetPassword(String username, String password,
		...
		VbLogin vbLogin = (VbLogin) session.createCriteria(VbLogin.class)
				...
				.uniqueResult();
		if(vbLogin != null){
			vbLogin.setPassword(PasswordEncryption.encryptPassword(password));
			vbLogin.setEnabled(OrganizationUtils.LOGIN_ENABLED);
			vbLogin.setFirstTime(OrganizationUtils.FIRST_TIME_LOGIN_YES);
			vbLogin.setWrongPasswordCount(new Integer(0));
	...
	public Boolean checkPassword(String username, String password,
			VbOrganization organization) {
		Session session  = this.getSession();
		Boolean isSimilarPassword = Boolean.FALSE;
		String existedPassword = null;
		VbLogin vbLogin = (VbLogin) session.createCriteria(VbLogin.class)
				.add(Restrictions.eq("username", username))
				.uniqueResult();
		if(vbLogin != null){
			existedPassword = vbLogin.getPassword();
			if(!existedPassword .equals(PasswordEncryption.encryptPassword(password))){
				resetPassword(username, existedPassword, organization);
			}else{
				isSimilarPassword = Boolean.TRUE;
			}
		}
		return isSimilarPassword; 
