		...
		try {
			...
			if (form instanceof SalesReturnCommand) {
				...
				if ("save-sales-returns".equals(action)) {
					...
					resultSuccess.setMessage(resultSuccessMessage);
					resultSuccess.setStatus(resultSuccessStatus);
				} else if ("get-customer-invoice-name-and-grid-data".equals(action)) {
					String businessName = request.getParameter("businessName");
					SalesReturnResult resultList = salesReturnDao.getInvoiceNameAndGridData(businessName, organization, userName);
