...
public class DayBookDao extends MySalesBaseDao {
	 ...
	 */
	private DayBookResult getCustomerTotalPayable(Session session, String salesExecutiveName,
	...
	public Float getOpeningBalance(String salesExecutive,
		...
		Float openingBal = getOpeningBalance(session, salesExecutive, organization);
		session.close();
	 ...
	 * @return openingBal - {@link Float}
	 */
	private Float getOpeningBalance(Session session, String salesExecutive,
			VbOrganization organization) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	private List<DayBookResult> getGridData(Session session, String salesExecutive,
	...
	public List<DayBookViewResult> getDayBookOnId(Integer id,
		...
		if (vbDayBook != null) {
			...
			if (!productList.isEmpty()) {
				...
				result.setAllowancesRemarks(StringUtil.format(amount.getAllowancesRemarks()));
				result.setAmountRemarks(StringUtil.format(amount.getAmountsRemarks()));
				result.setProductsRemarks(StringUtil.format(""));
				resultList.add(result);
			}
	 ...
	 * 
	 */
	private Boolean getAllotmentType(Session session, String userName, VbOrganization organization) {
	 ...
	 */
	public String getSalesExecutiveStartDate(String userName, VbOrganization organization) {
		Session session = this.getSession();
		String createDate = getSalesExecutiveStartDate(session, userName, organization);
		session.close();
	 ...
	 * @return createDate - {@link Date}
	 */
	private String getSalesExecutiveStartDate(Session session, String userName,
			VbOrganization organization) {
	...
	/**
	 * This method is responsible to get {@link VbDayBook} basic information.
	 * 
	 * @param userName - {@link String}
	 * @param organization - {@link VbOrganization}
	 * @return dayBookResult - {@link DayBookResult}
	 * @throws DataAccessException - {@link DataAccessException}
	 */
	public DayBookResult getDayBookData(String userName, VbOrganization organization) throws DataAccessException {
		Session session = null;
		try {
			session = this.getSession();
			DayBookResult dayBookResult = new DayBookResult();
			dayBookResult.setStartDate(getSalesExecutiveStartDate(session, userName, organization));
			dayBookResult.setOpeningBalance(getOpeningBalance(session, userName, organization));
			dayBookResult.setProductResults(getGridData(session, userName, organization));
			dayBookResult.setCustomerTotalPayable(getCustomerTotalPayable(session, userName, organization));
			dayBookResult.setAllotmentType(getAllotmentType(session, userName, organization));
