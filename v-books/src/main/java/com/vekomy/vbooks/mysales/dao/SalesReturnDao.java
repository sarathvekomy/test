...
public class SalesReturnDao extends MySalesBaseDao {
			...
			for (SalesReturnCommand salesReturnCommand : salesReturnCommandList) {
				...
				if(!new Float(0.00).equals(totalCost)) {
					...
					vbSalesBookProducts = (VbSalesBookProducts) session.createCriteria(VbSalesBookProducts.class)
							...
							.uniqueResult();
					if (vbSalesBookProducts != null && resalableProductQty != 0) {
					...
					}else if(vbSalesBookProducts == null && resalableProductQty != 0){
						 
						vbSalesBookProducts = new VbSalesBookProducts();
						vbSalesBookProducts.setQtyClosingBalance(resalableProductQty);
						vbSalesBookProducts.setQtyOpeningBalance(resalableProductQty);
						vbSalesBookProducts.setBatchNumber(batchNo);
						vbSalesBookProducts.setProductName(productName);
						vbSalesBookProducts.setVbSalesBook(salesBook);
						session.save(vbSalesBookProducts);
						if (_logger.isDebugEnabled()) {
							_logger.debug("Saving vbSalesBookProducts.");
						}
					}
				}
			}
	...
	/**
	 * This method is responsible to get the invoice name and grid data based.
	 * 
	 * @param businessName - {@link String}
	 * @param organization - {@link VbOrganization}
	 * @param userName - {@link String}
	 * @return salesReturnsResults - {@link SalesReturnsResult}
	 * @throws DataAccessException - {@link DataAccessException}
	 */
	public SalesReturnResult getInvoiceNameAndGridData(String businessName, VbOrganization organization, String userName) throws DataAccessException {
		SalesReturnResult srResult = new SalesReturnResult();
		List<SalesReturnsResult> salesReturnsResults = getGridData(businessName , organization, userName);
		srResult.setInvoiceName(getInvoiceName(businessName , organization));
		srResult.setSalesReturnResults(salesReturnsResults);
		return srResult;
	}
