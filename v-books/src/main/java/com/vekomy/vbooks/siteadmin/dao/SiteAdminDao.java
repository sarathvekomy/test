...
public class SiteAdminDao extends BaseDao {
	...
	public void saveOrganization(OrganizationCommand organizationCommand,
		...
		try {
			...
			if(vbOrganization != null){
				VbOrganization org=vbOrganization;
				parent=org.getId().toString();
				while(org != null){
					VbOrganization orgBranch=(VbOrganization) session.createCriteria(VbOrganizationMapping.class)
							                               .setProjection(Projections.property("vbOrganizationByMainBranchId"))
							                               .add(Restrictions.eq("vbOrganizationBySubBranchId", org))
							                               .uniqueResult();
					if(orgBranch != null){
						org=orgBranch;
						parent = parent+","+org.getId().toString();
					}else{
						org=null;
					}
            ...
            organizationMapping.setParent(parent);
			if (_logger.isDebugEnabled()) {
	...
	/**
	 * This method is responsible for checking main branch for imports.
	 * 
	 * @param mainBranchName - {@link OrganizationCommand}
	 * @return available-{@link Boolean}}
	 */
	public Boolean checkMainBranch(String mainBranchName){
		Boolean available = Boolean.FALSE;
		Session session=this.getSession();
		String orgName=(String) session.createCriteria(VbOrganization.class)
