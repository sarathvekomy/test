...
<%
	...
	try {
		...
		if (changeTransactionDao != null) {
			...
			Iterator deliveryNoteProductsIterator = vbDeliveryNote.getVbDeliveryNoteProductses().iterator();
			if (deliveryNoteProductsIterator.hasNext()) {
				vbDeliveryNoteProducts = (VbDeliveryNoteProducts) deliveryNoteProductsIterator.next();
			}
			...
			if (vbDeliveryNoteProducts == null) {
			 ...
			 grandTotal=changeTransactionDao.getGrandTotal(deliveryNoteId,user.getOrganization());
		}
	} catch (Exception exx) {
		exx.printStackTrace();
	}
					...
					<div class="form-row">
						...
						<div class="input-field-float">
							<input
								...
								name="totalCost" id="totalCost" readonly="readonly" value="<%=StringUtil.currencyFormat(grandTotal)%>">
						</div>
					</div>
...
TransactionChangeRequestHandler.deliveryNoteLoad();
	...
	$(document).ready(function() {
		...
		$('#branchname').hide();
		$('#chequeno').hide(); 
		$('#deliverynote-form').find('input,textarea,select').live('focus', function (event) {
		    var name  = this.name,
		        value = this.value,
		        id    = this.id,
		        dObj  = {};
		    dObj[id] = value;
		    TransactionChangeRequestHandler.oldDeliveryNoteBasicInfoFormValues(id,value);
		});
		//TransactionChangeRequestHandler.deliveryNoteGetPaymentTypes();
		//find all the form elements in your form,
		//then bind an event handler to all of the elements for the `change` event
		  $('#deliverynote-form').find('input,textarea,select').live('change paste', function (event) {
