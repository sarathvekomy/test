var ProductHandler = {
   ...
   initSearchResultButtons : function () {
				...
				//$('#product-edit-form').clearForm();
					...
					$.post('default.json','action=get-all-product-categories',function(obj){
						...
						for(var loop = 0; loop<result.length ; loop = loop +1 ){
							if($('#productCategoryName').val() != result[loop].value){
								$('#productCategoryName').append( $('<option></option').val(result[loop].value).text(result[loop].value));
							}
						}
					});
			...
			'</div>';
			 ...
			 for(var loop=0;loop<data.length;loop=loop+1) {
					 var productTableRows ='<div class="ui-content report-content">';
						...
						if(data[loop].productCategory == undefined || data[loop].productCategory == "select"  ){
	...
	getExistedProductCategories : function(){
		$.post('default.json','action=get-all-product-categories',function(obj){
			var result = obj.result.data;
			for(var loop = 0; loop<result.length ; loop = loop +1 ){
				$('#productCategoryName').append( $('<option></option').val(result[loop].value).text(result[loop].value));
