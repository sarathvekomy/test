var ManageUserHandler = {
	...
	initAddButtons : function() {
		...
		$('#action-clear').click(function() {
			//code for conformation message in ManageUser when click on clear button.
			...
			$("#error-message").dialog({
				...
				buttons: {
					'Ok' : function() {
						$(this).dialog('close');
						$(ManageUserHandler.manageUserSteps[ManageUserHandler.manageUserStepCount]).clear();
						$('#uValid').empty();
						$('#pValid').empty();
						$('#unValid').empty();
						$('#pwValid').empty();
		...
		$('#action-cancel').click(function() {
			...
			$("#error-message").dialog({
				...
				buttons: {
					'Ok' : function() {
						var paramString="action=remove-manage-user-session-data";
			        	$.post('manageUser.json',paramString,function(obj) {
							$('.task-page-container').html('');
			    			var container ='.manage-user-page-container';
			    			var url = "siteadmin/user/manage_user_list.jsp";
			    			$(container).load(url);
			        	});
		    			$(this).dialog('close');
					},
	...
	search : function(){
		var thisButton = $(this);
		var paramString="action=get-disabled-users-list";
		$.post('manageUser.json', paramString, function(obj){
		var userName = obj.result.data;
	        ...
	        function(data){
				...
				if(data != undefined) {
					...
					for(var loop=0;loop<data.length;loop=loop+1) {
						if(alternate) {
							var rowstr = '<div class="green-result-row alternate" id="user-row-'+data[loop].id +'"  style="height:53px !important;">';
						} else {
							rowstr = '<div class="green-result-row" id="user-row-'+data[loop].id +'"  style="height:53px !important;">';
						}
						alternate = !alternate;
						rowstr = rowstr + '<div class="green-result-col-1" style="height:50px !important;">'+
						...
						'</div>' +
						'<div class="green-result-col-2" style="height:50px !important;">'+
						'<div class="result-body">' +
						...
						'</div>' +
						'<div class="green-result-col-action" id="green-result-col-action-'+data[loop].id+'" style="height:41px !important;">'+
						...
						'<div id="'+data[loop].id+'" class="disable-icon enable-disable" title="Disable User"></div>' +
						'</div>'+
						'</div>';
						...
						if(data[loop].username == userName[i].disabledLoginUserName){
							 $("#user-row-"+ data[loop].id).css('opacity','0.5');
	...
	initUserListButtons : function(){
		$('.btn-view').click(function() {
			var id = $(this).attr('id');
			$.post('manageUser.json','action=get-employee-data&id='+id,function(obj) {
				var result = obj.result.data;
				$.post('siteadmin/user/manage_user_profile_view.jsp', 'id='+id, function(data){
					$('#user-view-container').html(data); 
					...
					$('#employeeEmail').text(result.employeeEmail);
					...
					$.post('manageUser.json', 'id='+id+'&action=get-assigned-user', function(obj) {
						var selectedOrg = obj.result.data;
						$('.organizationNameView').text(selectedOrg.toString());
					});
					$('.table-field').css({"width":"800px"});
					$('.main-table').css({"width":"400px"});
					$('.inner-table').css({"width":"400px"});
		...
		$('.edit-icon').click(function() {
			...
			$.post('manageUser.json','action=get-employee-data&id='+id,function(obj) {
				var result = obj.result.data;
				$.post('siteadmin/user/manage_user_edit.jsp', 'id='+id, function(data) {
					$('.manage-user-page-container').html(data);
					$("#id").val(id);
					var middleName = result.middleName;
					if(middleName != undefined) {
						$('#fullName').val(result.firstName+' '+middleName+' '+result.lastName);
					} else {
					...
					$('#addressType option').each(function() { 
						addressTypesArray.push( $(this).attr('value') );
					});
					if(addressTypesArray.length>0) {
						$('#addressType').empty();
						var select= 'select'
						for(var loop = 0;loop < addressTypesArray.length;loop = loop+1){
							if(addressTypeSelected == addressTypesArray[loop]){
								$('#addressType').append( $('<option></option').text(addressTypeSelected).attr('selected','selected'));
							}else{
								$('#addressType').append( $('<option></option').text(addressTypesArray[loop]));	
							}
						};
					}
					
					$.post('manageUser.json','action=get-organizations', function(data) {
						var orgList = data.result.data;
						$.post('manageUser.json', 'id='+id+'&action=get-assigned-user', function(obj) {
		...
		$('.disable-icon').click(function(){
			var manageUserId = $(this).attr('id');
				 var userStatusParam='Disabled';
			     $('#user-enable-disable-container').html('Are You Sure Want To Disable This User ?');
				  $("#user-enable-disable-dialog").dialog({
						autoOpen: true,
						height: 200,
						width: 500,
						modal: true,
						buttons: {
							Yes: function() {
								 $.post('manageUser.json', 'id='+manageUserId+'&action=modify-manage-user-status&userStatus='+userStatusParam, function(obj) {
									 ManageUserHandler.search();
									 $("#user-enable-disable-dialog").dialog('close');
								 });
							},
				            No: function() {
				            	$('#user-enable-disable-dialog').dialog('close');
		...
		//enable Manage User
		 $('.enable-icon').click(function(){
			  var manageUserId = $(this).attr('id');
			  var userStatusParam='Enabled';
			  $('#user-enable-disable-container').html('Are You Sure Want To Enable This User ?');
			  $("#user-enable-disable-dialog").dialog({
					autoOpen: true,
					height: 200,
					width: 500,
