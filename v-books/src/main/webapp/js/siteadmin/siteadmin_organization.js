var OrganizationHandler = {
	...
	initOrganizationListButtons: function() {
		...
		$('#organization-list').live('click',function(){
			$('.organization-page-container').load("siteadmin/organization_list.jsp");
		...
		$('.disable-icon').click(function(){
			...
			if($("#organization-row-"+ organizationId).hasClass('green-result-col-disabled-1')){
				 $("#organization-row-"+ organizationId).css("title","Enable Organization");
			     ...
			     $('#organization-list-container').html('Are You Sure Want To Enable This Organization ?');
				  $("#organization-list-dialog").dialog({
						...
						buttons: {
							Yes: function() {
								 $.post('organization.json', 'id='+organizationId+'&action=modify-organization-status&organizationStatus='+OrganizationStatusParam, function(obj) {
									 $("#organization-row-"+ organizationId).css("width","595px");
									 $("#organization-row-"+ organizationId).removeClass("green-result-col-disabled-1");
									 $("#green-result-col-action-"+ organizationId).find(".edit-icon").css("pointer-events", "visible");
									 $("#green-result-col-action-"+ organizationId).find(".btn-view").css("pointer-events", "visible");
			  ...
			  $("#organization-list-dialog").dialog({
					...
					buttons: {
						Yes: function() {
							 $.post('organization.json', 'id='+organizationId+'&action=modify-organization-status&organizationStatus='+OrganizationStatusParam, function(obj) {
								 $("#organization-row-"+ organizationId).css("width","595px");
								 $("#organization-row-"+ organizationId).addClass("green-result-col-disabled-1");
								 $("#green-result-col-action-"+ organizationId).find(".edit-icon").css("pointer-events", "none");
								 $("#green-result-col-action-"+ organizationId).find(".btn-view").css("pointer-events", "none");
								 $("#organization-list-dialog").dialog('close');
							 });
						},
			            No: function() {
	        ...
	        function(obj){
				...
				if(data != undefined) {
					...
					for(var loop=0;loop<data.length;loop=loop+1) {
						...
						'<div style="float: left; position: absolute;">'+
						'<span class="property" style="font: normal 13px arial;">Landmark</span>' +
						'</div>' +
						'<div style="margin-left: 87px;">'+
						'<span class="property-value" style="word-wrap:break-word;">' + data[loop].landmark +'</span>' +
						'</div>' +
						'</div>' +
						...
						'<div style="float: left; position: absolute;">'+
						'<span class="property" style="font: normal 13px arial;">Phone 2</span>' +
						'</div>' +
						'<div style="margin-left: 57px;">'+
						'<span class="property-value" style="word-wrap:break-word;">' + data[loop].phone2 +'</span>' +
						'</div>' +
						'<div style="float: left; position: absolute;">'+
						...
						'<div id="'+data[loop].id+'" style="margin-top:25px; float: left;" class="ui-btn disable-icon" title="Disable Organization"></div>' +
						'</div>' +
				        '</div>';
						...
						for(var i=0; i< organizationName.length; i=i+1) {
							if(data[loop].superUserName == organizationName[i].disabledLoginUserName){
								$("#organization-row-"+ data[loop].id).css("width","595px");
	...
	initSearchOrganization : function (role) {
		...
		$('#action-clear').click(function() {
			//$('#organization-search-form').clearForm();
			$('#organization-list').click();
			setTimeout(function(){
		...
		$('#action-search-organization').click(function() {
			var thisButton = $(this);
			var paramString = $('#organization-search-form').serialize();  
			$('#school-results-list').ajaxLoader();
			$.post('organization.json', paramString,
		        function(obj){
			    	var data = obj.result.data;
					$('#school-results-list').html('');
					if(data != undefined) {
						...
						for(var loop=0;loop<data.length;loop=loop+1) {
							...
							alternate = !alternate;
							rowstr = rowstr + '<div class="green-result-col-1" style="width:575px;">'+
							'<div class="result-title" style="width:575px;">'+
							...
							'</div>' +
							'<div class="green-result-col-action" id="green-result-col-action-'+data[loop].id+'" style="width: 82px; height:75px;padding: 13px 5px 2px 5px">' + 
							'<div id="'+data[loop].id+'" style="margin-top:25px; padding: 1px;float: left;" class="ui-btn edit-icon"></div>' +
							'<div id="'+data[loop].id+'" style="margin-top:25px; padding: 1px;float: left;" class="ui-btn btn-view" title="Load Organization"></div>'+
							'<div id="'+data[loop].id+'" style="margin-top:25px; float: left;" class="ui-btn disable-icon"></div>' +
					        '</div>';
							$('#school-results-list').append(rowstr);
				};
				OrganizationHandler.initOrganizationListButtons();
	...
	suggestOrganizationNames : function(thisInput,orgId) {
		...
		$.post('organization.json','action=get-organization-names&mainBranchName=' +mainBranchNameVal+'&id='+orgId,function(obj) {
			$.loadAnimation.end();
			suggestionsDiv.html('');
			var data = obj.result.data;
			if (data != undefined) {
				...
				suggestionsDiv.append('<div id="">' + 'No Organization Names' + '</div>');
			}
	...
	initOrganizationCreate: function() {
			...
			$("#error-message").dialog({
				...
				buttons: {
					'Ok' : function() {
		    			...
		    			var container ='.organization-page-container';
		    			var url = "siteadmin/organization_list.jsp";
		    			$(container).load(url);
		    			$(this).dialog('close');
		    			setTimeout(function(){
							$('#school-results-list').jScrollPane({showArrows:true});
		...
		$('#action-update').click(function(){
			$('.page-selection').css("width","183px");
			$('.page-container').css("width","700px");
			$('.page-link-strip').show();
			...
			$.post('organization.json', 'id='+id+'&action=update-organization',
		        function(data){
					$.post('siteadmin/organization_list.jsp', '',
				        function(data){
							$('.organization-page-container').html(data);     
						PageHandler.pageSelectionHidden =false;
							PageHandler.hidePageSelection();
