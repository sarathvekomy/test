...
suggestSalesExecutiveName : function(thisInput) {
	...
	$('.report-body').live("keyup",function(e){
		var code = e.keyCode || e.which;
		if(code !== 9){
			var id=$(this).attr('id');
			var availableQtyVal = currencyHandler.convertStringPatternToNumber($('#'+id).find('#availableQuantity').html());
			var qtyClosingBalance = currencyHandler.convertStringPatternToNumber($('#'+id).find('#qtyClosingBalance').html());
	        var quantityAllotted = currencyHandler.convertStringPatternToNumber($('#'+id).find('#quantityAllotted').val());
	        // alert(quantityAllotted);
	         var productName = $('#'+id).find('#productName').html();
	         var batchNumber = $('#'+id).find('#batchNumber').html();
	         var salesExecutive = $('#salesExecutive').val();
	         var thisId = $(this);
	         ...
	         $.post('accounts.json','action=get-existing-allotment&productName='+productName+'&batchNumber='+batchNumber+'&salesExecutive='+salesExecutive ,function(obj) {
	       	 ...
	       	 // if(parseInt(existingAllotment) != quantityAllotted) {
	       		  var sum= parseInt(qtyClosingBalance) + parseInt(quantityAllotted) + parseInt(existingAllotment);
			          ...
			          $('#'+id).find('#qtyOpeningBalance').val(currencyHandler.convertFloatToStringPattern(sum));
			        existingAllotment = "0";
	       	  //}
	         });
		}
...
/*	$('.grid-content').live("blur",function(){
	...
	$('#button-save').click(function() {
		 ...
		 SalesBookHandler.validationFlag = true;
		...
		$.post('accounts.json', 'action=is-sales-executive-available&salesExecutive='+salesExecutive, function(obj) {
			...
			if(isAvailable == 'true') {
				...
				$('div#allot-stock-grid').each(function(index, value) {
					  ...
					  var id = $(this).find('div').attr('id');
					  var availableQtyVal = currencyHandler.convertStringPatternToNumber($('#'+id).find('#availableQuantity').html());
					  var qtyAllotted = currencyHandler.convertStringPatternToNumber($('#'+id).find('#quantityAllotted').val());
					  var qtyOpeningStock = currencyHandler.convertStringPatternToNumber($('#'+id).find('#qtyOpeningBalance').val());
					  ...
					  if(parseFloat(qtyAllotted) < 0 || parseFloat(qtyAllotted) > parseFloat(availableQtyVal)) {
			         	  	...
			         	  	$.loadAnimation.end();
			         	  	SalesBookHandler.validationFlag = false;
			         	  	return false;
			           } else {
			        	   var qtyOpeningBalanceVal = currencyHandler.convertStringPatternToFloat($('#'+id).find('#qtyOpeningBalance').val());
							  var remarks = encodeURIComponent($('#'+id).find('.remarks').val());
							  var productName = $('#'+id).find('#productName').html();
							  var batchNumber = $('#'+id).find('#batchNumber').html();
							  var qtyClosingBalance = currencyHandler.convertStringPatternToNumber($('#'+id).find('#qtyClosingBalance').html());
				 ...
				 if(allotmentVal == 0 && openingStock == 0){
					 $('#error-message').html("You are alloting with No products Do you want to continue");
						$("#error-message").dialog(
										{
											resizable : false,
											height : 140,
					...
					/* $.loadAnimation.end();
					 ...
					 return;*/
					  ...
					  if(listOfObjects != ""&& SalesBookHandler.validationFlag !== false) {
					   SalesBookHandler.validationFlag = true;
							 $.post('accounts.json', "action="+action+'&listOfObjs='+listOfObjects+'&salesExecutive='+salesExecutive+'&previousClosingBal='+previousClosingBal+'&advance='+advance+'&openingBal='+openingBal+'&allotmentType='+allotmentType, function(obj){
								 $.loadAnimation.end();
									$('.accounts-page-container').load('accounts/sales-book/allot_stock.jsp');
					...
					$('div#allot-stock-grid').each(function(index, value) {
						  ...
						  if(parseFloat(qtyAllotted) < 0 || parseInt(qtyAllotted) > parseInt(availableQtyVal)) {
				        	  ...
				        	  var remarks = encodeURIComponent($('#'+id).find('.remarks').val());
				 			  ...
				 			  var batchNumber = $('#'+id).find('#batchNumber').html();
				 			  var qtyClosingBalance = currencyHandler.convertStringPatternToNumber($('#'+id).find('#qtyClosingBalance').html());
				 			  var availableQuantityVal = currencyHandler.convertStringPatternToNumber($('#'+id).find('#availableQuantity').html());
				 			  var qtyOpeningBalanceVal = currencyHandler.convertStringPatternToNumber($('#'+id).find('#qtyOpeningBalance').val());
				 			  var openingBlncFixed = currencyHandler.convertStringPatternToNumber($('#'+id).find('#openingBalanceFixed').val());
				 			  if(qtyOpeningBalanceVal !== 0 && qtyOpeningBalanceVal != openingBlncFixed) {
				 	        	  if(index == 0){
				 	        		  listOfObjects +=productName+'|'+remarks+'|'+qtyClosingBalance+'|'+qtyAllotted+'|'+qtyOpeningBalanceVal+'|'+batchNumber;
				 	        	  }else{
				 	        		  listOfObjects +='?'+productName+'|'+remarks+'|'+qtyClosingBalance+'|'+qtyAllotted+'|'+qtyOpeningBalanceVal+'|'+batchNumber;
 ...
 loadGrid: function(data) {
	 ...
	 if(data != undefined) {
		   ...
		   var count=1;
			...
			'<input type = "hidden" id="openingBalanceFixed"  size=6px style="border: none;margin-top:-3px; text-align:right !important;" readonly = "readonly" value="'+ currencyHandler.convertNumberToStringPattern(data[loop].qtyOpeningBalance) +'">'+
			...
			'<div id="'+data[loop].id+'" name="'+ currencyHandler.convertNumberToStringPattern(data[loop].availableQuantity) +'" class="edit-icon editAllotStock" title="Edit Allotment" style="margin-top:1px; display:none; float:right; margin-right:35px;"></div>' +
			'</div>'+
			'</div>';
 ...
 editAllotStock: function() {
	 //var availableQty = $('#availableQuantity').val();
		$('.editAllotStock').click(function() {
			
			var id = $(this).attr('id');
			var availableQty = $(this).attr('name');
			if("undefined" != id) {
				$.post('accounts/sales-book/allot_stock_edit.jsp', 'id='+id, function(data){
						...
						$("#allot-stock-edit-dialog").dialog({
			    			...
			    			Update: function() {
			    				...
			    				var integerRegex = /^\d+$/;
			    				if(integerRegex.test(qtyAllotted) === false || parseInt(qtyAllotted) > parseInt(availableQty)){
			    					showMessage({title:'Warning', msg:'Please Enter valid Quantity.'});
			    					return;
			    				}
			...
			$('#qtyAllotted').live("keyup",function(e){
				var integerRegex = /^\d+$/;
				var allotted= $(this).val();
				if(integerRegex.test(allotted) === false || parseInt(allotted) > parseInt(availableQty)){
					showMessage({title:'Warning', msg:'Please Enter valid Quantity.'});
					return;
				}
			});
		});
		/*$('#allot-stock-edit-form').live("keyup",function(e){
			alert($('#availableQuantity').val());
		});*/
		/*$('#allot-stock-edit-form').bind("input",function(e){
			...
			//var availableQuantity =$('#availableQuantity').val();
			//alert(qty);
			if(integerRegex.test(allotted) === false || allotted > qty){
				showMessage({title:'Warning', msg:'Please Enter valid Quantity.'});
				return;
			}
		});*/
