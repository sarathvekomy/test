...
var UserHandler = {
		...
		/*AjaxSucceeded: function(data1) {
	        if (data1.result.data == "y") {
	        	UserHandler.flag=true;
	        	  if(/^[a-zA-Z0-9\._]+$/.test($('#username').val())==false||($('#username').val()).length==0){
	        		  $('#uValid').html("<img src='"+THEMES_URL+"images/taken.gif' alt='Code already taken!'>");
	        		  $('#user_pop').css("visibility", "hidden");
	        		  UserHandler.flag=false;
	        		  $("#username").focus(function(){
	        			  $('#username_pop').css("visibility", "visible");
	                   	// $('#username_pop').show();
	                   	 $('#uValid').empty();
	                   });
	            $("#username").blur(function(event){
	           ...
	           // $('#uValid').html("<img src='"+THEMES_URL+"images/available.gif' alt='Username available!'>");
	            $('#password').val('');
	            $('#pValid').html("<span style=font-size: '"+10+"'px;>Enter Password!</span>");
	        	$('#password').focus();
	        } else if (data1.result.data == "v") {
	            $('#uValid').html("<img src='"+THEMES_URL+"images/available.gif' alt='FeeTypeCode available!'>");
	        	$('#description').focus();
	        ...
	        confirming and displaying remainder date is valid or Invalid
	            ...
	            $('#uValid').html("<img src='"+THEMES_URL+"images/taken.gif' alt='Code already taken!'>");
	            $('#username_pop').css("visibility", "hidden");
	            UserHandler.flag=false;
	                   $("#username").focus(function(){
	                	   if(/^[a-zA-Z0-9\._]+$/.test($('#username').val())==false||($('#username').val()).length==0){
	       	           	 $('#user_pop').hide();
		...
		initCheckPassword: function() {
			$("#password").blur(function(event) {
				/*if($('#pName').validate()==false) return;*/
				var uname=$('#username').val();
				var pwd=$('#password').val();
				 if(/^[a-zA-Z0-9\_.\@#$%^&*()-+\s]+$/.test($('#password').val())==false||pwd == uname||pwd.length<3){
					  UserHandler.flag=false;
			...
			$.post('default.json','action=get-address-types',function(obj){
				...
				if(result.length>0){
					...
					for(var loop=0;loop<result.length;loop = loop+1){
						$('#addressType').append( $('<option></option').text(result[loop]));
						if(result[loop]==type){
							loop=loop+1;
						}
						$('#addressTypeEdit').append( $('<option></option').text(result[loop]));	
				};
				}
			});
		},
		getPaymentTypes : function(){
			$.post('default.json','action=get-all-payment-types',function(obj){
				...
				if(result.length>0){
					...
					for(var loop=0;loop<result.length;loop = loop+1){
						$('#paymentType').append( $('<option></option').text(result[loop]));			
				};
				}
			});
		},
...
$.fn.pageLink = function(config) {
	...
	thisLink.click(function() {
		if(thisLink.activePagelink()===false) {
...
$.fn.ajaxLoader = function() {
	this.loadAnimation({image: {src: THEMES_URL+'images/common/ajax-loader.gif'}});	
}
