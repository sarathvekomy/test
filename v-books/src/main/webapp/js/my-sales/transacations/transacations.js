...
var DeliveryNoteHandler = {
	...
	// To Submit step data.
	saveStepData : function(sendFormData) {
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				// To enable or disable previous button.
	...
	initAddButtons : function() {
		...
		$('#button-next').click(function() {
			...
			if(DeliveryNoteHandler.deliverynoteStepCount != 1){
				var totalBalance=currencyHandler.convertFloatToStringPattern((totalPayableVal - currencyHandler.convertStringPatternToFloat(presentPaymentVal)).toFixed(2));
			...
			if (DeliveryNoteHandler.deliverynoteStepCount == 0) {
				...
				$.post('deliveryNote.json', 'action=check-businessname-availability&businessName='+businessName, function(obj) {
					...
					if(isExists == 'true'){
						...
						DeliveryNoteHandler.saveStepData(sendFormData);		
					} else {
	...
	loadGrid: function() {
        ...
        $('.green-results-list').html('');
			$.post('deliveryNote.json', 'action=get-grid-data&businessName=' +businessName, function(obj) {
				...
				$('#forPayments').attr('checked','checked');
...
var SalesReturnsHandler = {
		...
		suggestBusinessName : function(thisInput) {
			...
			$.post('salesReturn.json','action=get-business-name&businessNameVal=' + val,function(obj) {
				...
				/*	$("#businessName").autocomplete({
						  ...
						  var givenVal = $(this).val();
						  var existsVal = $.inArray(givenVal,data);
						  if (existsVal == 0) {
								  SalesReturnsHandler.getInvoiceNameAndGridData();
						  } else {
							  $('#invoiceName').val('');
							  $('.gridDisplay').hide();
						  }
		...
		getInvoiceNameAndGridData : function() {
			var businessName = $('#businessName').val();
			//Populating the invoice name and grid based on the business name selection.
			$.post('salesReturn.json','action=get-customer-invoice-name-and-grid-data&businessName='+businessName, function(obj) {
					$('#invoiceName').val(obj.result.data.invoiceName);
					
					$('.green-results-list1').html('');
					var data = obj.result.data.salesReturnResults;
					if(data !== undefined) {
						if(data.length>0) {
							$('.success-msg').hide();
							var count=1;
