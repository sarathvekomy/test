var ValidateChangeTransaction = {
		...
		validateReading : function(){
			...
			var dayBookType = 'Vehicle Fuel Expenses';
			var salesId=$('#salesBookId').val();
			$.ajax({type: "POST",
				url:'dayBookCr.json', 
				data: 'action=get-starting-reading&dayBookType='+dayBookType+'&salesBookId='+salesId,
				async : false,
				success: function(data){
					 existedStartReading = data.result.data;
					 return existedStartReading;
			...
			if(/^[0-9.,]+$/.test(endingReadingVal) == false || elen == 0 || endingReadingVal.charAt(0) == " " || endingReadingVal.charAt($('#endingReading').val().length-1)==" " || parseInt(endingReadingVal) <= parseInt(existedStartReading)){
