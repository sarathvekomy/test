var DayBookHandler = {
		...
		load:function() {
			var titleVal;
			$.post('dayBook.json', 'action=get-dayBook-data', function(obj) {
				var result = obj.result.data;
				// Populating allotStockOpeningBalance
				var allotStockOpeningBalVal = currencyHandler.convertStringPatternToFloat(result.openingBalance);
				...
				// Populating StartDate.
				$('#startDate').val(result.startDate);
				// Checking Allotment Type.
				var isDaily = result.allotmentType;
				if(isDaily == 'true') {
					$('#presentDate').hide();
				...
				// Populating Amounts.
				var customerTotalPayable = result.customerTotalPayable.presentPayable;
				var customerTotalReceived = result.customerTotalPayable.presentPayment;
				...
				// Populating Products Grid.
				$('.green-results-list').html('');
				var data = result.productResults;
				if(data != undefined){
					if(data.length>0) {
						var count=1;
...
initExpensesButtons : function(){
	...
	allowanceEdit : function(id) {
		...
		/*alert($('#'+mainId).children('.row2').children('.col2').attr('class'))*/
