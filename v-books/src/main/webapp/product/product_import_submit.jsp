	...
	try {
		...
		while (rows.hasNext()) {
			...
			if(isRowWrongValidated){
				...
				String brand=FileUploadUtil.readCell(row.getCell(2));
				String model=FileUploadUtil.readCell(row.getCell(3));
				String productName = brand + " " + model;
				String batchNumber = FileUploadUtil.readCell(row.getCell(4));
				Boolean isProductExists = productDao.isProductExists(productName, batchNumber, organization);
				if(!isProductExists) {
					productCommand = new ProductCommand();
					productCommand.setProductCategory(FileUploadUtil.readCell(row.getCell(1)));
					...
					productCommand.setDescription(FileUploadUtil.readCell(row.getCell(5)));
					productCommand.setCostPerQuantity(Float.parseFloat(FileUploadUtil.readCell(row.getCell(6))));
