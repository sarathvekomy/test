		...
		try{
			...
			if (form instanceof ChangeRequestDayBookBasicInfoCommand) {
				...
				if ("daybook-basic-info".equals(action)) {
					...
					String startReading = dayBookCrDao.getExistedStartReading(dayBookType,userName,getOrganization());
					
					resultSuccess.setData(startReading);
					resultSuccess.setMessage(resultSuccessMessage);
					resultSuccess.setStatus(resultSuccessStatus);
				} else if("get-allowances".equals(action)) {
					List<DayBookAllowancesResult> allowancesResultList = dayBookCrDao.getAllowances(userName, organization);
					...
					String dayBookType = request.getParameter("dayBookType");
					List<DayBookAllowancesResult> allowancesResultList = dayBookCrDao.getExecutiveAllowances(userName, organization,dayBookType);
