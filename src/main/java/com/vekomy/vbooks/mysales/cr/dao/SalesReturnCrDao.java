		...
		if (grandTotal > 0) {
			...
			while (grandTotal != 0) {
				...
				if (creditInfo != null) {
					...
					saveCustomerAdvanceInfo(session, salesReturnBusinessName, newInvoiceNo, vbOrganization, grandTotal, userName,invoiceNo);
	...
	private void persistSalesReturnCRDetails(Session session, String userName,
		...
		for (VbSalesReturnChangeRequestProducts changedProducts : productsSet) {
			...
			if (changedProducts.getTotalCost().contains(",1")) {
				instanceReturnProducts.setTotalCost(Float.parseFloat(changedProducts.getTotalCost().replace(",1", "")));
			} else {
				instanceReturnProducts.setTotalCost(Float.parseFloat(changedProducts.getTotalCost().replace(",0", "")));
			}
