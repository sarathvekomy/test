...
public class OrganizationDao extends BaseDao {
		...
		if (organization != null) {
			...
			try {
				...
				/*if ("Y".equalsIgnoreCase(organization.getMainBranch())) {*/
					...
					if(OrganizationUtils.LOGIN_ENABLED_STRING.equalsIgnoreCase(organizationStatus) && ! "Y".equalsIgnoreCase(organization.getMainBranch())){
						VbOrganizationMapping orgMainBranchMapping=(VbOrganizationMapping) session.createCriteria(VbOrganizationMapping.class)
								                                         .add(Restrictions.eq("vbOrganizationBySubBranchId", organization))
								                                         .uniqueResult();
						if(orgMainBranchMapping != null){
							List<VbLogin> loginIsEnabledList = (List<VbLogin>) session.createCriteria(VbLogin.class)
									                        .add(Restrictions.eq("vbOrganization", orgMainBranchMapping.getVbOrganizationByMainBranchId()))
									                        .add(Restrictions.eq("enabled",OrganizationUtils.LOGIN_ENABLED))
									                        .list();
							if( loginIsEnabledList.isEmpty()){
									String message = Msg.get(MsgEnum.ORGANIZATION_SUB_BRANCH_ENABLED_ERROR);
									throw new DisabledException(message);
							}
						}
					}
					// Disabling main branch login credentials, if it doesn't have sub-branches.
					enableOrDisableCredentials(session, organization, organization.getSuperUserName(), organizationStatus);
					
					// Disabling main branch employee credentials.
					List<VbEmployee> mainBranchEmployeeList = session.createCriteria(VbEmployee.class)
							.add(Expression.eq("vbOrganization", organization))
							.list();
					if(!mainBranchEmployeeList.isEmpty()) {
						for (VbEmployee vbEmployee : mainBranchEmployeeList) {
							enableOrDisableCredentials(session, organization, vbEmployee.getUsername(), organizationStatus);
						}
					}
					// For sub branches.
					List<VbOrganizationMapping> subBranchIdList = session.createCriteria(VbOrganizationMapping.class)
							.add(Restrictions.eq("vbOrganizationByMainBranchId", organization))
							.list();

					if (!subBranchIdList.isEmpty()) {
						for (VbOrganizationMapping subBranchId : subBranchIdList) {
							Integer subBranchOrgId = subBranchId.getVbOrganizationBySubBranchId().getId();
							VbOrganization subOrganization = (VbOrganization) session.get(VbOrganization.class, subBranchOrgId);
							
							// Disabling sub branch login credentials.
							enableOrDisableCredentials(session, subOrganization, subOrganization.getSuperUserName(), organizationStatus);
													
							// disable all employees of sub-branch
							List<VbEmployee> modifiedEmployeeList = (List<VbEmployee>) session.createCriteria(VbEmployee.class)
									.add(Expression.eq("vbOrganization", subOrganization))
									.list();
							
							if (modifiedEmployeeList != null && !modifiedEmployeeList.isEmpty()) {
								for (VbEmployee vbEmployee : modifiedEmployeeList) {
									enableOrDisableCredentials(session, subOrganization, vbEmployee.getUsername(), organizationStatus);
								} 
							} 
						}
					}  
				 /*else {
					if(OrganizationUtils.LOGIN_ENABLED_STRING.equalsIgnoreCase(organizationStatus)){
						VbOrganizationMapping orgMainBranchMapping=(VbOrganizationMapping) session.createCriteria(VbOrganizationMapping.class)
								                                         ...
								                                         .uniqueResult();
						if(orgMainBranchMapping != null){
							VbLogin loginIsEnabled = (VbLogin) session.createCriteria(VbLogin.class)
									                        .add(Restrictions.eq("vbOrganization", orgMainBranchMapping.getVbOrganizationByMainBranchId()))
									                        .add(Restrictions.eq("enabled",OrganizationUtils.LOGIN_ENABLED))
									                        .uniqueResult();
							if(loginIsEnabled == null){
									String message = Msg.get(MsgEnum.ORGANIZATION_SUB_BRANCH_ENABLED_ERROR);
									throw new DisabledException(message);
							}
						}
					}
					// Modifying organization credentials status.
					enableOrDisableCredentials(session, organization, organization.getSuperUserName(), organizationStatus);
					
					// disable all employee belongs to disabled organization
					List<VbEmployee> modifiedEmployeeList = (List<VbEmployee>) session.createCriteria(VbEmployee.class)
							.add(Expression.eq("vbOrganization", organization))
							.list();
					if (modifiedEmployeeList != null && !modifiedEmployeeList.isEmpty()) {
	 ...
	 */
	private void enableOrDisableCredentials(Session session, VbOrganization organization, String userName, String status) {
