	...
	$.post('employee.json', paramString, function(obj){
			...
			if(data != undefined) {
					...
					for(var loop=0;loop<data.length;loop=loop+1) {
						...
						'</div>' +
						'</div>' +
						'<div class="green-result-col-action" id="green-result-col-action-'+data[loop].id+'" style="width:95px !important;">';
						...
						if('Super Management' == data[loop].loginEmployeeType || 'MGT' == data[loop].loginEmployeeType){
							rowstr = rowstr+'<div id="'+data[loop].id+'" class="ui-btn btn-login-track" title="View Login Details" style="margin-top:0px; width:16px !important; margin-left:5px !important;"></div>';
						}
...
initSearchResultButtons : function () {
	$('.edit-icon').click(function() {
		...
		$('.btn-login-track').click(function() {
			var id = $(this).attr('id');
