var TransactionChangeRequestHandler = {
	...
	initDNSearchResultButtons : function() {
								...
								'Ok' : function() {
									...
									if(TransactionChangeRequestHandler.deliverynoteStepCount == 1){
										...
										/*$('select option').each(function(){
											if(originalPaymentType == $(this).attr('value')){
												$('#paymentType option[value="'+$(this).attr('value')+'"]').remove();
												$("#paymentType option[value=-1]").text(originalPaymentType);
												$("#paymentType option[value=-1]").val(originalPaymentType);
											}
										});*/
									}
									$(this).dialog('close');
		...
		dayBookInitAddButtons : function() {
			...
			$('#startReading').blur(function() {
				var startReading=currencyHandler.convertStringPatternToFloat($('#startReading').val());
				var fomatStartReading=currencyHandler.convertFloatToStringPattern(startReading.toFixed(2));
				$('#startReading').val(fomatStartReading);
			});
			$('#endingReading').blur(function() {
				var endingReading=currencyHandler.convertStringPatternToFloat($('#endingReading').val());
				var fomatEndingReading=currencyHandler.convertFloatToStringPattern(endingReading.toFixed(2));
				$('#endingReading').val(fomatEndingReading);
			});
		},
		...
		dayBookLoad:function() {
			...
			$('#vehicleFuelExpensesCRLink').click(function(){
				...
				d.dialog('open');
				$.post('dayBookCr.json','action=get-allowances-details&dayBookType='+dayBookType,function(obj){
					var result = obj.result.data;
		...
		populateAllowances : function() {
			$.post('dayBookCr.json', 'action=get-allowances', function(obj) {
				var data = obj.result.data;
