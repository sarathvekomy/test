/**
 * com.vekomy.vbooks.app.dao.base.BaseDao.java
 *
 ...
 */
package com.vekomy.vbooks.app.dao.base;
...
import com.vekomy.vbooks.app.utils.StringUtil;
...
public class BaseDao extends HibernateDaoSupport {
	...
	/**
	 * This method is responsible to get {@link VbOrganization} based on id.
	 * 
	 * @param session - {@link Session}
	 * @param organizationId - {@link Integer}
	 * @return organization - {@link VbOrganization}
	 */
	public VbOrganization getOrganization(Session session, Integer organizationId) {
	...
	public VbSalesBook getVbSalesBookNoFlag(Session session,  String userName, VbOrganization organization) {
		Query query = session.createQuery(
				"FROM VbSalesBook vb WHERE vb.vbOrganization = :vbOrganization1 AND vb.salesExecutive = :salesExecutiveName1 AND"
	...
	public String generateSalesBookNo(VbOrganization organization) {
		...
		VbInvoiceNoPeriod invoiceNoPeriod = (VbInvoiceNoPeriod) session.createCriteria(VbInvoiceNoPeriod.class)
				.add(Restrictions.eq("vbOrganization", organization))
				.add(Restrictions.eq("txnType", ApplicationConstants.TXN_TYPE_SALES_BOOK))
				.uniqueResult();
		StringBuffer generatedSalesBookNo = new StringBuffer();
		if (latestSalesBookNo == null) {
			if(invoiceNoPeriod != null) {
				generatedSalesBookNo.append(organization.getOrganizationCode())
				.append(seperator).append(ApplicationConstants.TXN_TYPE_SALES_BOOK)
	 ...
	 */
	public void saveSystemNotification(Session session,
